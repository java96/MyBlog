        ...
        <div class="result-wrap">
            <div class="result-title">
                <div class="result-list">
                    <a href="<%=basePath%>Backstage/diary/diaryAdd.jsp"><i class="icon-font"></i>添加日记</a>
                </div>
            </div>
            <div class="result-content">
                <table class="result-tab" width="100%">
                    ...
                    <s:iterator value="#request.listDiary" var="diary">
                        <tr>
                            <td>${id}</td>
                            <td>${diaryTitle}</td>
                            <td>${diaryRecommend}</td>
                            <td>
                                    <%--给日志长度做判断--%>
                                <s:if test="%{null!=diaryText&&diaryText.length()>70}">
                                    <s:property value="%{diaryText.substring(0, 70)}"/>
                                </s:if>
                                <s:else>
                                    <s:property value="%{diaryText}"/>
                                </s:else>
                            </td>
                            <td><s:date name="diaryTime" format="yyyy-MM-dd hh:mm"/></td>
                            <td>
                                <a href="diary_queryDiaryById?diary.id=${diary.id}">修改</a>
                                <a href="diary_deleteDiary?diary.id=${diary.id}">删除</a>
                            </td>
                        </tr>
                    </s:iterator>
                </table>
