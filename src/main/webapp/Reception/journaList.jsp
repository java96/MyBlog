...
<article class="blogs">
    <h1 class="t_nav"><a href="/" class="n1">网站首页</a><a href="/" class="n2">我的日志</a></h1>

    <div class="newblog left">
        <s:iterator value="#request.journalList.beanList">
            <h2>${journalTitle}</h2>
            ...
            <ul class="nlist">
                    ...
                    <%--给显示文字做判断--%>
                <s:if test="%{null!=journalText&&journalText.length()>300}">
                    <s:property value="%{journalText.substring(0, 300)}"/>
                </s:if>
                <s:else>
                    ${journalText}
                </s:else>
        ...
        <div class="page">
            <a href="<%=basePath%>journal_receptionJournal?pageBean.pc=1">&lt;&lt;</a>
            <s:if test="#request.journalList.pc > 1">
                <a href="<%=basePath%>journal_receptionJournal?pageBean.pc=${journalList.pc -1}">&lt;</a>
            </s:if>
            共${journalList.tp}页/当前第${journalList.pc}页
            <s:if test="#request.journalList.tp > #request.journalList.pc">
                <a href="<%=basePath%>journal_receptionJournal?pageBean.pc=${journalList.pc + 1}">&gt;</a>
            </s:if>
            <a href="<%=basePath%>journal_receptionJournal?pageBean.pc=${journalList.tp}">&gt;&gt;</a>
        </div>
    </div>
    ...
    <aside class="right">
        <div class="news">
            ...
            <ul class="rank">
                <s:iterator value="#request.journalRece">
                    <li><a href="<%=basePath%>journal_clickJournal?journal.id=${id}" title="${journalTitle}"
                           target="_blank">${journalTitle}</a></li>
                </s:iterator>
            </ul>
            <h3 class="ph">
                <p>点击排行</p>
            ...
            <ul class="paih">
                <s:iterator value="#request.journalClick">
                    <li><a href="<%=basePath%>journal_clickJournal?journal.id=${id}" title="${journalTitle}"
                           target="_blank">${journalTitle}</a></li>
                </s:iterator>
            </ul>
